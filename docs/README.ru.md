...
Project indexing строит структурированную карту definitions, references и files для flags, variables, scripted triggers/effects, focuses, events, GUI elements, GFX sprites, texture paths и localisation keys.
Project validation возвращает red/yellow/green checks для duplicate definitions, brace balance, missing textures or sprites, missing localisation keys и `replace_path` risks.
Project repair может работать в dry-run или применять fixes для encoding и formatting. Он проверяет UTF-8 BOM rules, script formatting, типы файлов в `sound/` и OGG metadata в `music/`. Если нужен ffmpeg, но он отсутствует, dry-run возвращает install guidance; после user approval `dry_run=false` и `install_ffmpeg=true` разрешают silent installation attempt.
Script editing изменяет existing HOI4 script files через replace или insert named blocks, с dry-run preview и brace checks.
Experimental GUI/GFX asset tool генерирует local procedural PNG assets, `.gfx` sprite registration и optional `.gui` files без external image models. Запись новых assets требует `approved=true`.
...
Пример project validation call:
...
Пример experimental GUI/GFX asset dry-run:
