...
Project repair может работать в dry-run или применять fixes для encoding и formatting. Он проверяет UTF-8 BOM rules, script formatting, типы файлов в `sound/` и OGG metadata в `music/`. Если нужен ffmpeg, но он отсутствует, RHoiScribe возвращает guidance по установке и не устанавливает его без user approval.
